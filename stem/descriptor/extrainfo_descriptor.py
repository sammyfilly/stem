...
def _parse_timestamp_and_interval(keyword: str, content: str) -> Tuple[datetime.datetime, int, str]:
  ...
  if not interval.isdigit():
    raise ValueError(f"{keyword} line's interval wasn't a number: {line}")
  ...
  try:
    ...
    raise ValueError(f"{keyword} line's timestamp wasn't parsable: {line}")
...
def _parse_extra_info_line(descriptor: 'stem.descriptor.Descriptor', entries: ENTRY_TYPE) -> None:
  ...
  if len(extra_info_comp) < 2:
    ...
    raise ValueError(
        f"Extra-info line entry isn't a valid nickname: {extra_info_comp[0]}")
  ...
  for value in _values('transport', entries):
    ...
    if ' ' not in value:
      ...
      if len(value_comp) < 1:
        ...
        raise ValueError(
            f"Transport line's address:port entry is missing a colon: transport {value}"
        )
...
def _parse_hs_stats(keyword: str, stat_attribute: str, extra_attribute: str, descriptor: 'stem.descriptor.Descriptor', entries: ENTRY_TYPE) -> None:
  ...
  if value is None:
    ...
    raise ValueError(f"'{keyword}' line was blank")
  else:
    ...
    try:
      ...
      raise ValueError(
          f"'{keyword}' stat was non-numeric ({stat_value}): {keyword} {value}"
      )
    ...
    if validate:
      for keyword in self._required_fields():
        if keyword not in entries:
          raise ValueError(f"Extra-info descriptor must have a '{keyword}' entry")

      for keyword in self._required_fields() + SINGLE_FIELDS:
        if keyword in entries and len(entries[keyword]) > 1:
          raise ValueError(
              f"The '{keyword}' entry can only appear once in an extra-info descriptor"
          )

      expected_first_keyword = self._first_keyword()
      if expected_first_keyword and expected_first_keyword != list(entries.keys())[0]:
        raise ValueError(
            f"Extra-info descriptor must start with a '{expected_first_keyword}' entry"
        )
