...
def _parse_file(tordnsel_file: BinaryIO, validate: bool = False, **kwargs: Any) -> Iterator['stem.descriptor.tordnsel.TorDNSEL']:
  ...
  if kwargs:
    raise ValueError(f"TorDNSEL doesn't support additional arguments: {kwargs}")
    ...
    for keyword, values in list(entries.items()):
      ...
      if keyword == 'ExitNode':
        ...
        try:
          ...
          if validate:
            raise ValueError(f"Published time wasn't parsable: {value}")
      elif keyword == 'LastStatus':
        try:
          self.last_status = stem.util.str_tools._parse_timestamp(value, datetime.timezone.utc)
        except ValueError:
          if validate:
            raise ValueError(f"LastStatus time wasn't parsable: {value}")
        ...
        for value, block_type, block_content in values:
          ...
          if validate:
            if not stem.util.connection.is_valid_ipv4_address(address):
              raise ValueError(f"ExitAddress isn't a valid IPv4 address: {address}")
            elif block_content:
              raise ValueError(f'Unexpected block content: {block_content}')
          ...
          try:
            ...
            if validate:
              raise ValueError(f"ExitAddress found time wasn't parsable: {value}")
      elif validate:
